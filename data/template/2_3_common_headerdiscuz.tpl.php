...
0
|| checktplrefresh('./template/lcb_geek/common/headerdiscuz.htm', './template/lcb_geek/common/header_common.htm', 1508167049, '3', './data/template/2_3_common_headerdiscuz.tpl.php', './template/lcb_geek', 'common/headerdiscuz')
|| checktplrefresh('./template/lcb_geek/common/headerdiscuz.htm', './template/default/common/header_qmenu.htm', 1508167049, '3', './data/template/2_3_common_headerdiscuz.tpl.php', './template/lcb_geek', 'common/headerdiscuz')
|| checktplrefresh('./template/lcb_geek/common/headerdiscuz.htm', './template/lcb_geek/common/erjidaohang.htm', 1508167049, '3', './data/template/2_3_common_headerdiscuz.tpl.php', './template/lcb_geek', 'common/headerdiscuz')
|| checktplrefresh('./template/lcb_geek/common/headerdiscuz.htm', './template/default/common/pubsearchform.htm', 1508167049, '3', './data/template/2_3_common_headerdiscuz.tpl.php', './template/lcb_geek', 'common/headerdiscuz')
;?>
...
<?php } ?>
</head><meta property="qc:admins" content="455132372764164116376763757672556000" />
...
<div id="toptb" class="cl">
  ...
  <div class="wp">
    <div class="z">
      <?php if(is_array($_G['setting']['topnavs']['0'])) foreach($_G['setting']['topnavs']['0'] as $nav) { ?>      <?php if($nav['available'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1))) { ?>
      <?php echo $nav['code'];?>
      <?php } ?>
      <?php } ?>
    ...
    <div class="y"> <a id="switchblind" href="javascript:;" onClick="toggleBlind(this)" title="开启辅助访问" class="switchblind">开启辅助访问</a>
      ...
      <a href="javascript:;" id="switchwidth" onClick="widthauto(this)" title="<?php if(widthauto()) { ?>切换到窄版<?php } else { ?>切换到宽版<?php } ?>" class="switchwidth">
      <?php if(widthauto()) { ?>
      切换到窄版
      <?php } else { ?>
      切换到宽版
...
<ul id="myprompt_menu" class="p_pop" style="display: none;">
  ...
  <li><a href="home.php?mod=follow&amp;do=follower"><em class="prompt_follower<?php if(empty($_G['member']['newprompt_num']['follower'])) { ?>_0<?php } ?>"></em>
    新听众
    <?php if($_G['member']['newprompt_num']['follower']) { ?>(<?php echo $_G['member']['newprompt_num']['follower'];?>)<?php } ?></a></li>
  <?php if($_G['member']['newprompt'] && $_G['member']['newprompt_num']['follow']) { ?>
  <li><a href="home.php?mod=follow"><em class="prompt_concern"></em>
    我关注的
...
<div id="sslct_menu" class="cl p_pop" style="display: none;">
  <?php if(!$_G['style']['defaultextstyle']) { ?>
  <span class="sslct_btn" onClick="extstyle('')" title="默认"><i></i></span>
  <?php } ?>
...
</div><?php } ?><?php echo adshow("headerbanner/wp a_h");?><style>
.itk_tm a:hover {
color:#356190;
...
.itk_nav {
float:left; height:75px; line-height:75px; width:60px;text-align:center; padding:0 5px 0 20px;font-size:16px;display:block;
}
.itk_tm li span {display:none}
</style>


<div id="hd" style="border-bottom:1px solid #e5e5e5;background:#fff; width:100%;margin-bottom:10px;">
  <div class="wp">
    <div class="hdc cl">
      <?php $mnid = getcurrentnav();?>      <h2 style="padding:0px; margin:6px 0 0 0;">
...
<ul class="itk_tm" style="width:530px;height:75px;overflow:hidden;margin-bottom:-75px"><?php if(is_array($_G['setting']['navs'])) foreach($_G['setting']['navs'] as $nav) { if($nav['available'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1))) { ?>
<li class="itk_nav" <?php echo $nav['nav'];?>></li><?php } ?>
 <?php } ?>
</ul>
...
<body>
<div id="dzf" class="cl" style="border-top:1px solid #cdcdcd;">
<style type="text/css">
#dzf{padding:15px 0;line-height:160%;}
#dzf p{width:295px;text-indent:80px;text-align: left;position:relative;}
.dzfbl{border-left:1px solid #FFF;}
.dzfbr{border-right:1px solid #DDD;}
#dzf em{display:block;color:#666;}
#dzf .xg2{color:#666;}
#dzf em a{color:#369;letter-spacing:0px;}
#dzf .ver{color:#666;}
</style>
        <p class="z dzfbr" style="background:url(<?php echo $_G['style']['styleimgdir'];?>/nv2-1.png) no-repeat 10px 2px"> <a href="#" target="_blank" class="xw1">在线重装系统工具 (v1.0.5.10)</a> <em class="xg2">无需技术 傻瓜式操作 系统想换就换</em> <em> <a href="#" target="_blank">软件详情</a> | <a href="#">BUG提交</a> | <a href="#">使用交流</a> </em> </p>
        <p class="z dzfbr dzfbl" style="background:url(<?php echo $_G['style']['styleimgdir'];?>/nv2-2.png) no-repeat 10px 2px"> <a href="#" target="_blank" class="xw1">蓝天维护系统工具箱 (v1.1.1.1)</a> <em class="xg2">维护员工具，界面简洁漂亮实用</em> <em> <a href="#" target="_blank">详情</a> | <a href="#" target="_blank">BUG提交</a> | <a href="#" target="_blank">使用交流</a> | </em> </p>
        <p class="z dzfbr dzfbl" style="background:url(<?php echo $_G['style']['styleimgdir'];?>/nv2-3.png) no-repeat 10px 2px"> <a href="#" target="_blank" class="xw1">U盘装机工具 (v1.2.0.8)</a> <em class="xg2">电脑店必备的系统安装维护环境</em> <em> <a href="#" target="_blank">软件详情</a> | <a href="#" target="_blank">BUG提交</a> | <a href="#" target="_blank">使用交流</a> </em> </p>
        <p class="z dzfbl" style="background:url(<?php echo $_G['style']['styleimgdir'];?>/nv2-4.png) no-repeat 10px 2px"> <a href="#" target="_blank" class="xw1">常用软件安装器 (v1.0.4.2)</a> <em> <a href="#" target="_blank">BUG提交</a> | <a href="#" target="_blank">使用交流</a> | <a href="#" target="_blank">软件详情</a> </em> <em> <a href="#" target="_blank">IT资讯精选</a> | <a href="#" target="_blank">墙纸主题</a> | <a href="#" target="_blank">县赏问答</a> </em> </p>
      </div>

</body>
</html></div><?php echo adshow("subnavbanner/a_mu");?><?php if($_G['setting']['search']) { $slist = array();?><?php if($_G['fid'] && $_G['forum']['status'] != 3 && $mod != 'group') { ?><?php
$slist[forumfid] = <<<EOF
